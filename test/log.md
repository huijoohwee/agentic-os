...
v229 (2026-09-09): latest Graph full registry v225 is **4,840 passed / 23 failed / 4,863 total**; five recovered, zero new failures versus v223. Source check and independent standalone exports pass; the mixed historical docs fixture remains explicitly scoped. v226 Game FPS browser passes, including active/inactive shared NPC highlights, mobile input and save recovery; six filesystem list requests stayed within the declared canonical seed directory. v227 XR units pass 188/188, plus all six XR/video source validation commands. v228 keeps all 86 native source-runner tests passing while sparse Git fixtures materialize 11 rather than 7,661 files: 68,731,928 fewer bytes per fixture, with identical full committed trees. These are local proofs and observed efficiency measurements; Graph remains unreleased, XR browser checks remain pending, and independent Commerce runtime evidence is incomplete. Exact receipts: commerce-efficiency-resolution-v225.md through commerce-efficiency-resolution-v228.md in the 2026-09-08 session evidence directory.

v135 (2026-09-08): upstream failure observations now optionally identify one reported prerequisite by repository/path. Advisory cross-source prerequisite groups preserve separate source partitions, unmapped cases, and unlisted counts; no path reads, execution, coverage inference, or readiness authority. Latest Graph v134: **4,647 passed / 211 failed / 4,858 total**, 213.605 seconds, one added passing property-bag regression, zero recovered existing cases or new failures. Source check and independent exports passed with stable bytes. 41 unchanged checks report missing docs; legacy mirror docs are sealed by Graph’s source release inventory, so no downstream rename was performed. Graph remains unreleased; Commerce runtime readiness remains unresolved. See `commerce-efficiency-resolution-v134.md` and `commerce-efficiency-resolution-v135.md` for exact evidence and source-release status.

Graph validation — v132 (2026-09-08): full source-root `npm test` completed **4,640 passed / 217 failed / 4,857 total** in **214.292 seconds** (exit 1). Six stale geospatial checks recovered; no new failures or omitted cases versus v127. The one reserved product test file follows the current component owners and executes shared surface-ownership/insertion-placement behavior instead of asserting removed implementations. Focused geospatial comparison: **112/124 → 118/124**. Source, canonical, pinned docs, native reservation and four standalone artifact bytes remained stable during the full run. This is validation repair, not a production bug-fix or measured harness speedup; Graph remains unreleased. Receipts: `graph-v132-validation-manifest.json`; handoff: `commerce-efficiency-resolution-v132.md`.

Upstream Podman fix: [workerd #7270](https://github.com/cloudflare/workerd/pull/7270), candidate `bab79723766c342aedd9ce42dd7ca5699af59797`, makes unset memory swappiness optional. Two compiled KJ tests pass; the original schema fails the regression. Actual schema-generated Podman requests change from start HTTP 500 to 204 and exit 0. Full Bazel validation was blocked by disk space; upstream merge still requires the contributor CLA and maintainer-assisted CI. The exact reproducer passed, its six temporary containers were removed, and Podman was stopped. Commerce remains unreleased with independent runtime evidence unresolved. No Prod mirror or Cloudflare deployment occurred. Evidence: `workerd-upstream-manifest.json`.
