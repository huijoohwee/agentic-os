/** CLI invocation policy and digest-fenced lifecycle catalog; pure grammar lives in src/invocation.mjs. */
...
export const ENTRY_CONTRACTS = deepFreeze({
  ...
  '/workflow.export': { kind: 'command', action: 'workflow', argv: ['export'], semantic: 'read-only', accepts: ['input'], requires: ['input'] },
...
function parseToken(token, declaration = false) {
  const parsed = parseInvocationToken(token);
  if (parsed.error) return parsed;
  if (!declaration && parsed.argument === '') return { error: 'argument-empty' };
  if (declaration && parsed.argument !== null && parsed.argument !== '') return { error: 'catalog-argument' };
  return parsed;
}
