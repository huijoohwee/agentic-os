...
test('the packaged fixed tool surface is deterministic and deeply frozen', () => {
  assert.deepEqual(TOOLS.map((tool) => tool.name), ['workflow.targets', 'workflow.collect', 'workflow.export', 'run.start', 'run.status', 'run.cancel', 'run.retry', 'run.query', 'run.trace', 'run.evaluate', 'run.compare', 'capabilities', 'collaborate', 'doctor', 'status', 'checks', 'reap', 'lane']);
