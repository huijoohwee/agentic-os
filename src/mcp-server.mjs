...
const WORKFLOW_TOOLS = invocationCatalog.entries.filter(entry => entry.action === 'workflow').map(entry => ({
  name: entry.token.slice(1), description: entry.summary, inputSchema: entry.token === '/workflow.targets' ? EMPTY_INPUT
    : { ...CHECKS_INPUT, properties: { input: { ...CHECKS_INPUT.properties.input, description: 'Exact local lifecycle manifest path; collection retains digest-bound native receipts.' }, ...(entry.token === '/workflow.export' ? { offset: { type: 'integer', minimum: 0, multipleOf: 32 } } : {}) } }, outputSchema: CLI_OUTPUT,
...
export function toolArguments(name, args) {
  ...
  if (name === 'checks' || name === 'workflow.collect' || name === 'workflow.export' || name === 'workflow.recommend') {
    if (!plainObject(args) || !onlyKeys(args, name === 'workflow.export' ? ['input', 'offset'] : ['input']) || typeof args.input !== 'string'
    ...
    return name === 'checks' ? ['observe', '--checks', `--input=${args.input}`]
      : ['workflow', name.slice(9), `--input=${args.input}`, ...(args.offset === undefined ? [] : [`--offset=${args.offset}`])];
  }
  if (name === 'reap') {
    const value = args === undefined ? {} : args;
