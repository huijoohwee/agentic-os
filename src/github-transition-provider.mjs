...
export function createGitHubTransitionRestProvider({ repository: identity,
  ...
  return Object.freeze({
    ...
    async prepareIntegrationProof(inputValue) {
      ...
      const initial = createGitHubAuthorityReadProvider({ issuance: input.predecessorIssuance,
    ...
    async observeProof(inputValue, expectedProof = null) {
      ...
      const initial = createGitHubAuthorityReadProvider({
        issuance: prior.stored.operationInput.predecessorIssuance, token, fetchImpl, timeoutMs });
