...
| `@orchestration-graph` | Source-backed state, node, edge, and stop-condition topology. | `SKILLS.md`, `HARNESS-CONTRACTS.md`, and existing Canvas/Agentic OS graph owners. | No external graph runtime import, hidden node registry, or direct graph-store mutation. |
...
| `/ecs.session-start #agentic-ecs @source.frontmatter @ecs-session` | Bind validated Agentic OS source to one private bounded ECS session. |
