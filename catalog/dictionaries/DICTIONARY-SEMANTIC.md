...
| `#learning` | Offline procedural Python learning scoped to the active workspace document and exact lesson/run identity. | `/python.learning @canvas #learning` discovers one browser-local inspection or explicitly requested control operation. | Graph owns its bounded worker, original lessons, shared rubric, native scene and persistence. Stale identities fail closed; run and debrief save are separate actions. UI remains usable when tools are absent. |
...
| `#procedural-asset` | Editable native Three.js assets from bounded text intent and typed procedural construction. | `/asset.create @text` resolves to the Graph construction owner through the shared consumer invocation route. | Exact source/recipe/seed/control values, stable parts and separate geometry, material, hierarchy, control and export checks accompany the asset. Typed controls rebuild locally without model calls; faithful GLB retains supported scene data and clips while procedural source remains a workspace companion. Text evidence never fabricates image/reference or provider approval; image-source validation remains unchanged. Metadata and deterministic validation do not prove runtime readiness or authorize execution. |
